...
(function () {
  ...
  d3.csv('RenewalComms.csv').then(function (data) {
    ...
    dayCounts.forEach(function (day) {
      day.percent = day.sent === 0 ? 0 : day.opened / day.sent;
    });
    ...
    x.domain(days);
    y.domain([0, d3.max(dayCounts, function (d) {
      return d.sent;
    })]);
    // y.domain([0, 0.5])
    ...
    sentBars.selectAll('rect').data(dayCounts).enter().append('rect').attr('x', function (d) {
      return x(d.day);
    }).attr('y', function (d) {
      return y(d.sent);
    ...
    openBars.selectAll('rect').data(dayCounts).enter().append('rect').attr('x', function (d) {
      return x(d.day);
    }).attr('y', function (d) {
      return y(d.opened);
    }).attr('width', x.bandwidth()).attr('height', function (d) {
    ...
    d3.select('#j-dg1').on('click', function () {
      d3.select('#j-dg1').attr('class', 'control active');
      d3.select('#j-dg2').attr('class', 'control');
    ...
    d3.select('#j-dg2').on('click', function () {
      d3.select('#j-dg1').attr('class', 'control');
      d3.select('#j-dg2').attr('class', 'control active');
